...
from backend.managers import ConnectionManager, DependencyManager, DeviceManager, SessionManager
from backend.models import DependencyRuleCreate, WebSocketMessage
...
mqtt_service = MQTTService(device_manager, session_manager, connection_manager, dependency_manager)
        ...
        try:
            ...
            for device_id in removed_ids:
                ...
                await connection_manager.broadcast(
                    WebSocketMessage(event="device_removed", data={"device_id": device_id})
                )
...
async def get_dependencies():
    return JSONResponse([rule.model_dump(mode='json') for rule in dependency_manager.list_rules()])


@app.post("/api/dependencies")
async def create_dependency(rule_create: DependencyRuleCreate):
    rule = dependency_manager.add_rule(rule_create)
    await connection_manager.broadcast(
        WebSocketMessage(event="dependencies_updated", data={"rules": [r.model_dump(mode='json') for r in dependency_manager.list_rules()]})
    )
    return JSONResponse(rule.model_dump(mode='json'))


@app.delete("/api/dependencies/{rule_id}")
async def delete_dependency(rule_id: str):
    removed = dependency_manager.delete_rule(rule_id)
    if not removed:
    ...
    return JSONResponse({"ok": True, "deleted_rule_id": rule_id})
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        await websocket.send_json({"event": "devices_data", "data": device_manager.get_all_devices()})
        ...
        await websocket.send_json(
            {
                "event": "dependencies_updated",
                "data": {"rules": [r.model_dump(mode='json') for r in dependency_manager.list_rules()]},
            }
        )
        ...
        while True:
            ...
            if event_type == "device_selected":
            ...
            elif event_type == "create_dependency":
                rule_create = DependencyRuleCreate(**payload)
                dependency_manager.add_rule(rule_create)
                await connection_manager.broadcast(
                    WebSocketMessage(
                        event="dependencies_updated",
                        data={"rules": [r.model_dump(mode='json') for r in dependency_manager.list_rules()]},
            ...
            elif event_type == "delete_dependency":
                rule_id = payload.get("rule_id", "")
                dependency_manager.delete_rule(rule_id)
                await connection_manager.broadcast(
                    WebSocketMessage(
                        event="dependencies_updated",
                        data={"rules": [r.model_dump(mode='json') for r in dependency_manager.list_rules()]},
                ...
                await connection_manager.broadcast(
                    WebSocketMessage(event="session_status", data={"active": True, "action": "started"})
                )
                await connection_manager.broadcast(
                    WebSocketMessage(event="session_matrix_update", data=matrix.model_dump(mode='json'))
                )
                ...
                await connection_manager.broadcast(
                    WebSocketMessage(event="session_status", data={"active": False, "action": "stopped"})
                )
                await connection_manager.broadcast(
                    WebSocketMessage(event="session_matrix_update", data={"devices": [], "matrix": []})
                )
